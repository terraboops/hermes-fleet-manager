...
| Ask the operator to **decide/act** (blocked) | `NEEDS-INPUT-<NAME>` | Use when you're blocked on a human decision or action. Put the question in plain text too (there is no pop-up for it). Set `state: awaiting_input`. |
...
- **Surface decisions plainly**: your work is managed, so when we need the operator, say the question in plain words + `NEEDS-INPUT`. Assume no interactive prompt exists.
