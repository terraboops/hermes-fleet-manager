...
reach the tool and no error surfaces. Repro (2026-09-17): wolfgang was doing the operate
end-to-end test pass with no `operate` MCP registered, discovered it mid-task, and had to ask the
...
PROFILES: dict[str, dict] = {
    "work": {
        "config_dir": f"{HOME}/.claude-work",
        "servers": {
            "operate": {
                "command": "node",
                "args": [f"{HOME}/Developer/operate/src/mcp/stdio.ts"],
                "env": {"OPERATE_API_URL": "https://operate.boreray-eel.ts.net"},
                "env_files": {"OPERATE_TOKEN": f"{HOME}/.config/operate/token"},
                "scope": "user",
            },
        },
    },
    "personal": {
        "config_dir": f"{HOME}/.claude-personal",
        "servers": {},
    },
}
...
def resolve(target: str | None) -> tuple[str | None, str]:
    ...
    if not target:
        return None, f"{HOME}/.claude-work"
