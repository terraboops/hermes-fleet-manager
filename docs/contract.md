...
- **The controller starts a timer at inject.** If the matching
  ...
  the operator, not silently retried): the session is flagged "dispatch not acked,"
...
When the controller spawns a NEW session or TAKES OVER one the operator opened
