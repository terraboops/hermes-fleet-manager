...
- **`fleet_overwatch.py arm` is now RE-ARM SAFE (2026-09-17).** Re-arming a session removes its
  ...
  `--focus-file` and re-arm. Verified: re-arming leaves exactly one `wolfgang-overwatch` job and the
...
- **`fleet_mcp.py` — MCP PROVISIONING AT LAUNCH (2026-09-17).** The launchers
  ...
  session simply could not reach the tool and no error surfaced (repro: wolfgang ran the operate
  end-to-end test pass with no `operate` MCP registered and had to ask the human for the add
  ...
  removing `operate` from the work profile and confirming `ensure_for()` restored it with a valid
